...
export function Maintenance({ g, stats, act }: { g: GameState; stats: PlantStats; act: GameApi["act"] }) {
  const f0 = stats.furnace;
  const hasRepairer = g.workers.some((w) => w.role === "vedlikehold");
  const canPlan = hasResearch(g, "vedlikeholdsplan");
  return (
    <Card title="Vedlikehold">
      <p className="g-muted">
        Foringen slites for hver charge. Planlagt stans koster {fmtKr(f0.relineCost)} og {f0.relineHours} timer. Brenner den
        gjennom, blir det havari: {fmtKr(f0.relineCost * 3)}, {f0.relineHours * 3} timer og tapt omdømme.
      </p>
      {g.furnaces.map((f, i) => {
