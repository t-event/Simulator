...
export function ResearchPage({ g, stats, act, onQuit, openBook, onLoadBackup }: Props) {
  ...
  const [tab, setTab] = useState<"forskning" | "bank" | "spillet">("forskning");
  const ready = researchOptions(g).filter((r) => r.available).length;
    ...
    <div className="g-grid">
      <div className="g-col-wide">
        <SubTabs
          ...
          onChange={setTab}
          tabs={[
            { id: "forskning", label: ready ? `Forskning (${ready})` : "Forskning" },
            { id: "bank", label: "Bank" },
            { id: "spillet", label: "Innstillinger" },
          ]}
        />
        {tab === "forskning" && <Research g={g} act={act} openBook={openBook} />}
        ...
        {tab === "bank" && (
          <Card title="Banken">
            ...
            <p className="g-muted">
              Rente {fmtPct(LOAN_INTEREST_PER_DAY * 365, 0)} per år, trukket daglig. Banken låner mer til større verk
              med godt omdømme. Kassekreditt: {fmtKr(creditLimit(g))} – blir du stående under den i en uke, er det
              konkurs.
            </p>
            <div className="g-row">
