-- B-170: topplista viser hvilken spilldag et spill ble koblet til kontoen («koblet til på dag N»). Den som spilte lenge
-- uten konto før innloggingen, har ellers en rask vekst på lista som ser rar ut for de andre. Dagen er den første i
-- tidslinja: i sesongen den første i sesongen, på «Alle tider» den første i det hele tatt.
-- Som i 016, med kolonnen linked_day til slutt. my_rank lages på nytt som i 016.
...
returns table (plass int, nickname text, value numeric, day int, is_me boolean, league text, stage smallint,
               honor text, title text, linked_day int)
language sql
stable
security definer
